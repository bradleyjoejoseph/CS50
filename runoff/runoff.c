...
bool vote(int voter, int rank, string name)
{
    ...
    {
        if (strcmp(candidates[q].name, name) == 0)
        {
            preferences[voter][rank] = q;
            return true;
        }
    }
    // TODO
...
bool print_winner(void)
{
    int winnerVoteAmount = voter_count / 2;
    for (int x = 0; x < candidate_count; x++)
    {
        if (candidates[x].votes > winnerVoteAmount)
        {
            printf("%s\n", candidates[x].name);
            return true;
        }
...
int find_min(void)
{
    ...
    for (int j = 0; j < candidate_count; j++)
    {
        ...
        {
            if (candidates[j].votes < smallestVotes)
            {
                smallestVotes = candidates[j].votes;
            }
        }
    }
    // TODO
